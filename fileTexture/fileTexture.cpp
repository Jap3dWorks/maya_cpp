...
public:
    ...
    void getCustomMappings(MHWRender::MAttributeParameterMappingList& mapping) override;
...
private:
    ...
    mutable MString fResolvedMapName;  // can be modiffied in constant declarations
...
FileNode::FileNode(): fWidth(0), fHeight(0) {}
...
MStatus FileNode::compute(const MPlug& plug, MDataBlock& block)
{
    ...
    MFloatVector resultColor(0.f, 0.f, 0.f);
    float resultAlpha = 1.0f;
    
    // read from file if wee need to
    if (!fImage.pixels())
    {
        MString& fileName = block.inputValue(aFileName).asString();
        MString exactName(fileName);

        // this class is derived from MPxNode doesn't has path
    ...
    if (data && fWidth > 0 && fHeight > 0)
    {
        ...
        //min max clamp
        float u = uv[0]; if (u < 0.f) u = 0.f; if (u > 1.f) u = 1.f;
        float v = uv[1]; if (v < 0.f) v = 0.f; if (v > 1.f) v = 1.f;
        
        static const size_t pixelSize = 4;  // bytes
        ...
        resultColor[0] = ((float)pixel[0]) / 255.f;
        resultColor[1] = ((float)pixel[1]) / 255.f;
        resultColor[2] = ((float)pixel[2]) / 255.f;
        resultAlpha = ((float)pixel[3]) / 255.f;
    }

    // set ouptut color attribute
...
MStatus FileNode::initialize()
{
    ...
    aCMConfigPath = tAttr.create("colorManagementConfigFilePath", "cmcfp",
    ...
    aCMWorkingSpace = tAttr.create("workingSpace", "ws", MFnData::kString,
        theStringCMWorkingSpace);
    MAKE_INPUT(tAttr);
    ...
    MAKE_INPUT(nAttr);
    aCMConfigEnabled = nAttr.create("colorManagementConfigEnabled", "cmce", MFnNumericData::kBoolean);
    MAKE_INPUT(nAttr);
    ...
    fResolvedSamplerName("")
{
    ...
    if (theRenderer)
    {
        ...
        if (fragmentMgr)
        {
            ...
            if (!fragAdded)
            {
                fragAdded = (sFragmentName == fragmentMgr->
                    addShadeFragmentFromBuffer(sFragmentBody, false));
            }
            if (!structAdded)
            {
                structAdded = (sFragmentOutputName ==
                    fragmentMgr->addShadeFragmentFromBuffer(sFragmentOutputBody, false));
            }
            if (!graphAdded)
            {
                graphAdded = (sFragmentGraphName ==
                    fragmentMgr->addFragmentGraphFromBuffer(sFragmentGraphBody));
            }

            // If we have them all, use final graph for the override
...
{
    // Pull the file name from the DG for use un updateShader
    MStatus status;
    ...
    if (fResolvedMapName.length() > 0 && fResolvedSamplerName.length() > 0)
    {
        ...
        if (renderer)
        {
            ...
            if (textureManager)
            {
                ...
                if (texture)
                {
                    MHWRender::MTextureAssignment textureAssigment;
                    textureAssigment.texture = texture;
                    shader.setParameter(fResolvedMapName, textureAssigment);
...
{
    const MString UserClassify("texture/2d:drawdb/shader/2d/fileTexture");
...
MStatus uninitializePlugin(MObject obj)
{
    ...
    CHECK_MSTATUS(
        MHWRender::MDrawRegistry::deregisterShadingNodeOverrideCreator(
            "drawdb/shader/testure/2d/fileTexture",
            sRegistrantId
        )
    );
