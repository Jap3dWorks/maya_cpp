...
MStatus RBFNode::initialize()
{
    ...
    aOutputRotate =
        nAttr.create("outputRotate", "outputRotate",
    ...
    aOutputRotateZ = uAttr.create("outputRotateZ", "outputRotateZ", MFnUnitAttribute::kAngle);
    aOutputRotate =
        nAttr.create("outputRotate", "outputRotate", aOutputRotateX, aOutputRotateY,
            aOutputRotateZ);
    nAttr.setArray(true);
    nAttr.setUsesArrayDataBuilder(true);
    nAttr.setWritable(false);
    ...
    aInputRestQuats =
        nAttr.create("inputRestQuat", "inputRestQuat", 
            MFnNumericData::k4Double, 0.0, &status);
    ...
    aInputValueCount = 
        nAttr.create("inputValueCount", "inputValueCount", MFnNumericData::kLong);
    addAttribute(aInputValueCount);
    affects(aInputValueCount);

    aInputQuatCount = 
        nAttr.create("inputQuatCount", "inputQuatCount", MFnNumericData::kLong);
    ...
    aOutputQuatCount = nAttr.create("outputQuatCount", "outputQuatCount",
    ...
    aRegularization = nAttr.create("reularization", "regularization", 
...
MStatus RBFNode::preEvaluation(const MDGContext& context,
...
MStatus RBFNode::compute(const MPlug& plug, MDataBlock& data)
{
    ...
    int outputCount = data.inputValue(aOutputQuatCount).asLong();
    ...
    if (dirty_)
    {
        ...
        status = buildFeatureMatrix(data, inputCount, outputCount, inputQuatCount, 
            outputQuatCount, rbf, radius, inputRestQuats);
        CHECK_MSTATUS_AND_RETURN_IT(status);
        dirty_ = false;
    }
    ...
    for (auto& quatMatrix : allQuats)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (outputQuats.size() && outputQuats[0].cols() > 1)
            {
                if (outputMode == 1)
                {
                    // for each output quat, store them all in to a single column matrix.
                    // the first column will contain the neutral quaternion which is assued
                    // to be the first column of each solver outputQuat matrix
                ...
                {
                    quatMatrix.block(0, col, 4, outputQuats[quatIndex].cols()) =
                        outputQuats[quatIndex];
    ...
    VectorXd allWeigths = VectorXd::Zero(cols);
    int col = outputMode == 0 ? 0 : 1;
    for (int i = 0; i < 3; ++i)
    {
        VectorXd scalars;
        ...
        if (weights.size() && outputQuatCount)
        {
            if (outputMode == 0)
            {
                allWeigths.segment(col, weights.size()) = weights;
                col += weights.size();
            }
            else
            {
                int nonNeutralCount = weights.size() - 1;
                allWeigths.segment(col, nonNeutralCount) = weights.tail(nonNeutralCount);
                col += nonNeutralCount;
            }
        }
        if (scalars.size())
    ...
    if (outputQuatCount)
    {
        if (outputMode == 1)
        {
            double sum = allWeigths.sum();
            if (sum < 1.0)
            {
                allWeigths[0] = 1.0 - sum;
            }
        }
        allWeigths.normalize();
    }
    ...
    {
        VectorXd outQ = averageQuaternion(allQuats[i], allWeigths);
    ...
    double radius, const std::vector<MQuaternion>& inputRestQuats)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        // store quats as column matrix so we can use wighted quaternion averaging
        ...
        solvers_[i].setFeatures(inputs, inputQuats[i], outputs, outQuats, rbf,
            radius, regularization, space[i]);
    }
    return MS::kSuccess;
}
