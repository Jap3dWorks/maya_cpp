...
void LinearRegressionSolver::setFeatures(
    const MatrixXd& featureMatrix,
    const std::vector<std::vector<MQuaternion>>& featureQuatMatrix,
    const MatrixXd& outputScalarMatrix,
    const std::vector<MatrixXd>& outputQuats,
...
)
{
    ...
    if (inputQuatCount)
    {
        ...
        double minFallof = std::numeric_limits<double>::max();
        ...
        // insert rotational distances to main distance mantrix
        int quatIndex = 0;
        for (auto& rd : mQuat)
        {
            for (int i = 0; i < sampleCount; ++i)
            {
                applyRbf(rd.block(0, i * 2, sampleCount, 2),
                    rbf_,
                    sampleRadius_[i] * radius_);
            }
...
{
    int sampleCount = featureMatrix_.rows() ? featureMatrix_.rows() : featureMatrix_.size();
    if (sampleCount <= 1)
    {
        return VectorXd();
    }
    ...
    if (inputCount)
    {
        for (int i = 0; i < inputCount; ++i)
        {
            if (featureNorms_[i] != 0.0) inputs[i] /= featureNorms_[i];
        }
    ...
    if (featureQuatMatrix_.size())
    {
        ...
        for (int i = 0; i < inputQuatCount; ++i)
        {
            ...
            for (int s1 = 0; s1 < sampleCount; ++s1)
            {
                ...
                applyRbf(inputDistance.segment(startIdx, inputQuatCount * 2),
                    rbf_, sampleRadius_[s1]);
            }
        }
    }
...
double swingTwistDistance(const MQuaternion& q1,
...
void decomposeSwingTwist(const MQuaternion& q,
    MQuaternion& swing,
    MQuaternion& twist)
{
    twist.x = q.x;
