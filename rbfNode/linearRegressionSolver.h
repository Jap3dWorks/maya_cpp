...
void decomposeSwingTwist(const MQuaternion& q, MQuaternion& swing,
    MQuaternion& twist);

void swingTwistDistance(const MQuaternion& q1, const MQuaternion& q2,
    double& swingDistance, double& twistDistance);
...
public:
    ...
    VectorXd solve(const VectorXd& inputs,
...
struct InverseMultiQuadraticBiharmonic {
    ...
    const double operator()(const double& x) const
    {
        return 1.0 / sqrt((x*x) + (r*r));
    }
    double r;
};

struct BeckerWendlandC2Basics {
    BeckerWendlandC2Basics(const double& radius)
    {
        r = radius > 0.0 ? radius : 0.001;
    }
    const double operator()(const double& x) const
